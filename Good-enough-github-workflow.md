...
To collaborate with others, you first need to add them as collaborators if your GitHub repository is private. Then, your collaborator needs to `git clone` your repository to their machine in their preferred location (using terminal to `cd` into the directory where they want the repo to live). Your collaborator can now make their own branch using `git checkout -b new-branch-name`, make the repo an RStudio project if working in RStudio and get to work. Adding and committing changes *locally* is exactly as described in step 7. The difference is when pushing to your repo, your collaborator needs to replace `origin` with `your-github-id/name-of-your-repo`, e.g., `git push corinne-riddell/InfantMortality new-branch-name` if you were a collaborator on that project with me. Then, you can merge the pull request after viewing the changes on GitHub.
