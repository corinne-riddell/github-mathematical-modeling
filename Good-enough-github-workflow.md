...
To collaborate with others, you first need to add them as collaborators if your GitHub repository is private. Then, your collaborator needs to `git clone` your repository to their machine in their preferred location (using terminal to `cd` into the directory where they want the repo to live). Your collaborator can now make their own branch using `git checkout -b new-branch-name`, make the repo an RStudio project if working in RStudio and get to work. `git add`ing, `git commit`ting and `git push`ing works the same as described in step 7. 
